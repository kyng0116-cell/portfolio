...
if tab == "💡 인사이트":
    ...
    with st.expander("대시보드", expanded=True):
        st.markdown("""
        <div style="overflow: hidden; width: 1496px; height: 1712.5px;">
            <div style="
    ...
    st.markdown("""<div class="section-box"><h3>분석 과정</h3><p>
                <h4>1. 데이터 엔지니어링 및 무결성 확보</h4><br/><p> 
                <strong>이미지 제약 극복을 위한 3-Source OCR 구축:</strong> 상세페이지가 이미지로 제공되어 
                ...
                식약처 DB의 기능성 인증 제품 간의 정합성을 높이기 위해 900여 제품을 수기 검수했습니다. 
                또한 추출된 성분 데이터를 식약처·EU 기준 알레 르기 유발 물질 정보와 다이소 상품의 성분 데이터 간 수기 검수 후 결합하여 
                안전성 리스크가 있는 제품군을 선별했습니다.<br/>
                <strong>3단 정규화 DB 설계:</strong> 중복이 많은 기존 Wide Table을 Core(기본),
                 Stats(판매 통계), Specs(성분·기능성)로 3단 분리 설계하여 30만 건 이상의 대용량 데이터 
                처리 효율과 참조 무결성을 극대화했습니다.</p><br/><br/>
                <h4>2. 고도화된 분석 방법론 적용</h4><br/><p>
                ...
                통계적으로 도출했습니다.<br/>
                <strong>인과추론 기반의 기능성 여부 별 매출 기여도 정밀 분석:</strong> PSM(성향점수매칭), IPTW(역확률 가 중치), OW(중첩 가중치) 
                세 가지 방법의 인과추론을 적용하여 공산성을 보이는 세 요소인 브랜드, 카테고리, 기능성 여부에 
                대한 심도있는 분석을 시행했습니다.<br/>
                <strong>하이브리드 감성 라벨링:</strong> GPT-4o-mini의 대량 처리 속도와 
                Human-in-the-loop을 결합해 30만 건의 리뷰를 1차 라벨링하고, 수동 구축한 골든셋으로 
                교차 검증하여 95% 이상의 신뢰도를 확보했습니다.</p><br/><br/>
                <h4>3. 모델 및 알고리즘 최적화</h4><br/><p>
                ...
                표준화(z-score) 합산하는 수요 밀도 점수 알고리즘을 자체 설계했습니다.</p></p></div>""", unsafe_allow_html=True)
    st.markdown("""<div class="section-box"><h3>주요 인사이트</h3><br/>
                <p><h4>1. 16.7%의 연착륙 법칙과 기초 스킨케어의 저력</h4><br/><p>
                ...
                <strong>인과적 통찰:</strong> 인과추론(PSM/IPTW/OW) 결과, '기능성 성분'은 단독으로 매출을 
                일으키기보다 기존의 카테고리, 브랜드와 결합할 때 효과가 폭발적으로 증폭되는 
                '조절 변수(Moderator)'로 작용합니다.</p><br/><br/>
                <h4>2. 관광 상권의 수요 이원화와 K-뷰티 스킨케어의 폭발성</h4><br/><p>
                ...
                '연착륙 스킨케어'의 재고 수요가 일반 상권 대비 압도적으로 높습니다.</p></p></div>""", unsafe_allow_html=True)
    st.markdown("""<div class="section-box"><h3>최적화 전략</h3><p>
                <h4>1. Safety: OCR 기반 전성분 필터링 시스템</h4><br/><p>
                <strong>문제점:</strong> 다이소몰 내 상품 정보가 주로 이미지 형태로 제공되어 자동 필터링에 한계가 있습니다.<br/>
                <strong>해결 방안:</strong> OCR 파이프라인으로 전성분을 정형 데이터로 추출하고, 식약처·EU 기준 DB와 
                실시간 교차 검증하여 리스크 성분을 자동 스크리닝합니다.<br/>
                <strong>기대 효과:</strong> 소비자 불신 해소 및 시각장애인 정보 접근성 향상을 통한 ESG 경영 실천을
                기대할 수 있습니다.</p><br/><br/>
                <h4>2. Revenue: '연착륙 스킨케어' 집중 및 타겟 마케팅</h4><br/><p>
                <strong>타겟 집중:</strong> 전체 뷰티 매출의 32.5%를 책임지는 연착륙 제품(16.7%) 중 
                83.3%가 스킨케어임을 확인, 5,000원 기초 제품을 핵심 상품군으로 육성합니다.<br/>
                <strong>협업 제안:</strong> 라운드랩과 다이소의 새로운 전용 라인 '마라도'를 제안 합니다. 마라도의 청정을
                담은 '저자극·수분·데일리' 케어로 피부의 기초를 채우고, 구매 경험이 곧 '우리 바다 살리기'로
                이어지는 가치를 전달합니다. 점차 사라져가는 해녀를 지키고 해양 생태계를 복원하는 후원
                사업을 통해,C커머스와는 다른 고객들의 감성을 충족시키는 하나의 브랜드가 될 수 있습니다.</p><br/><br/>
                <h4>3. Efficiency: GIS 기반 Hub & Spoke 이원화 물류</h4><br/><p>
                <strong>문제점:</strong> 외국인 밀집 상권에서 인기 스킨케어의 조기 품절 현상이 발생하고 있습니다.<br/>
                <strong>해결 방안:</strong> GIS 데이터로 외국인 트래픽 상위 5% 매장을 Hub로 지정하고 
                연착륙 스킨케어 재고 심도를 일반 매장 대비 획기적으로 높입니다.</p><br/><br/>
                <h4>4. 입점 테스트 시뮬레이션: 머신러닝 기반 매출 예측</h4><br/><p>
                <strong>문제점:</strong> 100% 사입 구조로 입점 실패 시 브랜드 신뢰도 하락과 재고 부담이 직결됩니다.<br/>
                <strong>해결 방안:</strong> 30만 건의 정제된 리뷰 데이터를 바탕으로 입점 전 단계에서 제품의 연착륙 확률과 
                예상 매출을 정량적으로 시뮬레이션합니다.</p></p></div>""", unsafe_allow_html=True)
