...
if tab == "💡 인사이트":
    ...
    st.markdown("""<div class="section-box"><h3>주요 인사이트</h3><p>
                <h4>1. 채널 영향력: 사회적 증거와 가용성 휴리스틱을 통한 전환율 극대화<h4><br/>
                ...
                <h4>2. 리워드 조건: 손실 회피 성향과 심리적 진입 장벽의 상관관계<h4><br/>
                <strong>분석 결과:</strong> 절대적인 할인 금액($5 등)의 규모보다, 혜택 수혜를 위해 요구되는 
                '최소 지불 금액'이 낮을 때 프로모션 완료율이 유의미하게 높게 나타났습니다.<br/>
                <storng>원인 해석:</strong> 고객은 혜택의 획득보다 즉각적인 지출을 '손실'로 더 민감하게 인지하는 
                '손실 회피(Loss Aversion)' 경향을 보입니다. 따라서 할인 폭의 절대값을 확대하는 것보다 
                최소 결제액을 낮추어 참여의 심리적 문턱(진입 장벽)을 완화하는 방식이 최종 전환에 
                더 효과적임을 확인했습니다.<br/><br/>
                <h4>3. 운영 전략: 현재 편향과 희소성 메시지의 결합 효과</h4><br/>
                <strong>분석 결과:</strong> 1+1 혜택(BOGO) 프로모션의 경우, 유효기간을 7일에서 5일로 
                단축했을 때 완료율이 약 10%p 상승하는 역설적 성과가 나타났습니다.<br/>
                <strong>원인 해석:</strong> 이는 보상을 지연시키지 않으려는 '현재 편향(Present Bias)'과 
                마감 임박에 따른 '희소성(Scarcity)' 메시지가 결합된 결과입니다. 
                유효기간의 단축이 오히려 고객의 고민 시간을 통제하고 즉각적인 행동을 유발하는 
                강력한 트리거(Nudge)로 작용했습니다.<br/><br/>""", unsafe_allow_html=True)
