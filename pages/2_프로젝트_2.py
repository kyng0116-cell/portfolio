...
if tab == "💡 인사이트":
    ...
    st.markdown("""<div class="section-box"><h3>분석 배경 및 목적</h3><br/><p><strong>해결하려는 문제:</strong> : 결과 중심적 분석에서 프로세스 중심의 행동 전환 분석으로의 전환.<br/>
                ...
                분석 목적: 고객 행동 여정 기반의 프로모션 효율 최적화 및 설계 표준 수립<br/>
    ...
    st.markdown("""<div class="section-box"><h3>분석 과정</h3><p>
                ...
                <h4>2. 전처리</h4><br/> 
                <strong>목적별 이원화 정제:</strong> 고객 행동 패턴을 온전히 보존하기 위한 '군집 분석용(결측치 1.01% 제거)'과 변수 간 비교의 신뢰도를 높이기 위한 '통계 해석용(결측치 6.46% 제거)'으로 데이터를 분리하여 정제 효율을 높였습니다.<br/>
                <strong>전환(Success) 정의:</strong> 단순 구매 발생이 아닌, 시간 순서상 '수신 → 확인 → 완료'의 행동 흐름이 유효하게 성립된 경우만을 진짜 '성과'로 판정하는 파생변수를 생성했습니다.<br/><br/>
                <h4>3. 분석</h4><br/>
                <strong>K-means 군집화:</strong> 엘보우 방법(Elbow Method)을 통해 최적 군집 수(K=4)를 결정하고, 행동 지표(조회율, 완료율, 이탈률)를 기준으로 고객을 4개의 세그먼트(충성전환형, 설득가능전환형, 관심관망형, 저반응이탈형)로 세분화했습니다.<br/>
                <strong>통계적 검증:</strong> 비모수 검정(Kruskal-Wallis)을 통해 군집 간 지표 분포 차이의 유의성을 확인하고, 카이제곱 검정으로 프로모션 속성을 통제한 상태에서 채널 자체가 미치는 강력한 영향력을 통계적으로 입증했습니다.<br/><br/></p></div>""", unsafe_allow_html=True)
    st.markdown("""<div class="section-box"><h3>주요 인사이트</h3><p>
                <h4>1. 사회적 증거와 가용성 휴리스틱 (채널의 힘)</h4><br/>
