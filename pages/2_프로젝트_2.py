...
if tab == "💡 인사이트":
    ...
    st.markdown("""<div class="section-box"><h3>분석 과정</h3><p><br/>
                <h4>1. 데이터 통합 및 최적화</h4><br/><p> 
                고객의 행동 여정을 입체적으로 파악하기 위해 프로모션·행동 로그·데모그래픽 데이터를 연동한 
                통합 데이터를 설계했습니다. 세그먼트 분석 데이터셋과 시각화 전용 데이터셋을 
                분리 구조화함으로써, 데이터 처리 속도와 분석의 정확도를 동시에 확보했습니다.</p><br/><br/>
                <h4>2. 데이터 전처리 및 피처 엔지니어링</h4><br/><p>
                ...
                <strong>분석 범위의 정밀화:</strong> 분석 결과의 신뢰도와 유의미성을 확보하기 위해 연령 범위를 79세 이하로 
                설정하고, 이에 해당하는 3,277건(19.28%)의 데이터를 최종 분석 집단으로 추출했습니다.<br/>
                <strong>성과 지표 재정의:</strong> 시간 흐름에 따른 '수신 → 확인 → 완료'의 유효 경로가 성립된 
                케이스만을 진성 전환으로 판정하는 파생 변수를 생성했습니다.</p><br/><br/>
                <h4>3. 다각적 행동 분석</h4><br/><p>
                ...
                의사결정 근거로서의 신뢰성을 확보했습니다.</p></p></div>""", unsafe_allow_html=True)
    st.markdown("""<div class="section-box"><h3>주요 인사이트</h3><p>
                <h4>1. 핵심 고객의 소비 지형: '생활권 중심'의 중장년층 행동 패턴</h4><br/><p>
                ...
                강력한 전환 트리거로 작용합니다.</p><br/><br/>
                <h4>2. 리워드 설계의 핵심: 고액 혜택보다 '낮은 실질 지불액'과 '즉시 체감'</h4><br/><p>
                <strong>분석 결과:</strong>할인 금액이 큰 오퍼보다 실질 지불액이 낮은 오퍼의 반응이 좋았습니다. 
                실질 지불액 2,000원인 오퍼(31.94%)의 완료율이 7,000원인 오퍼(11.23%) 
                대비 약 3배 가깝게 높게 관측되었습니다.<br/>
                <strong>원인 해석:</strong> 고객은 혜택의 기쁨보다 지출에 더 민감한 '손실 회피(Loss Aversion)' 
                경향을 가지므로, 절대적인 할인 폭보다 초기 결제 부담을 낮추는 것이 전환 유도에 훨씬 효과적입니다.</p><br/><br/>
                <h4>3. 채널 영향력: 사회적 증거와 가용성 휴리스틱을 통한 전환율 극대화</h4><br/><p>
                ...
                신속한 의사결정을 유도합니다.</p><br/><br/>
                <h4>4. 다중 채널 시너지: 반복 노출을 통한 전환 성과 극대화</h4><br/><p>
                <strong>분석 결과:</strong>접점 채널(Touchpoint)이 늘어날수록 완료율이 선형적으로 증가하여,
                 3개 채널 중복 노출 시 완료율은 50.00%로 단일 채널(20.44%) 대비 약 2.4배의 전환 상승 효과를 보였습니다.<br/>
                <strong>원인 해석:</strong> 반복적인 메시지 전달은 '단순 노출 효과(Mere-exposure Effect)'를 발생시켜 
                신뢰감을 형성하고 심리적 장벽을 낮추어 최종 행동 전환을 가속화합니다.</p></p></div>""", unsafe_allow_html=True)
    st.markdown("""<div class="section-box"><h3>최적화 전략</h3><p>
                <h4>1. 하이퍼 로컬(Hyper-local) 기반 타겟팅 전략</h4><br/><p>
                ...
                즉각적으로 이끌어냅니다.</p><br/><br/>
                <h4>2. 실질 지불 부담 최소화 방식의 리워드 설계</h4><br/><p>
                ...
                극대화합니다.</p><br/><br/>
                <h4>3. SNS 중심의 다중 채널 믹스(Multi-channel Mix) 운영</h4><br/><p>
