...
# ── 사이드바: 프로필 카드와 동일한 딥 슬레이트 퍼플 ─────────────
with st.sidebar:
    st.markdown("""
    <style>
    /* 사이드바 배경 — 프로필 카드와 완전 통일 */
    ...
    section[data-testid="stSidebar"] * { color: #e8e4f3 !important; }
    section[data-testid="stSidebar"] hr { border-color: rgba(232,228,243,0.15) !important; margin: 0.6rem 0 !important; }
    section[data-testid="stSidebar"] .stButton > button {
        background: transparent !important;
        border: 1px solid rgba(232,228,243,0.2) !important;
    ...
    section[data-testid="stSidebar"] .stButton > button:hover {
        background: rgba(232,228,243,0.1) !important;
        border-color: rgba(232,228,243,0.4) !important;
    }
...
.profile-card {
    ...
    background-image: linear-gradient(135deg, #3e3355 0%, #4a3d68 100%);
    border-radius: 1.2rem;
    margin-bottom: 2rem;
    box-shadow: 0 4px 24px rgba(30,20,60,0.16);
    border: none;
    height: auto;
...
/* ── 프로필 인포 컬럼 ── */
.profile-cols {
    display: flex; gap: 1rem; margin-top: 0.5rem; flex-wrap: wrap; width: 100%;
}
.profile-col {
...
.profile-col a:hover { color: #fff !important; }
...
.section-title {
    ...
    border-bottom: 2px solid #6b5b9e;
...
.card-box {
    ...
    box-shadow: 0 1px 8px rgba(60,45,100,0.06);
}
...
.skill-card {
    ...
    box-shadow: 0 1px 5px rgba(60,45,100,0.06);
    border: 1px solid #ece8f5;
    display: flex;
    align-items: center;
    gap: 0.9rem;
}
.skill-icon { font-size: 1.3rem; width: 1.8rem; text-align: center; }
.skill-name {
    font-weight: 600;
    font-size: 0.97rem;
...
/* 펼쳐보기 힌트 */
.hint { font-size: 0.84rem; color: #a899cc; margin-left: 0.7rem; }
...
.proj-card {
    ...
    border-radius: 0.9rem;
    padding: 1.5rem;
    box-shadow: 0 1px 8px rgba(60,45,100,0.07);
    border: 1px solid #ece8f5;
    min-height: 320px;
}
</style>
...
<div class="profile-card">
    <img src="https://ca.slack-edge.com/T088AB0N865-U09EHSCUNSF-3bef5911dc38-512"
         style="width:115px; height:115px; border-radius:50%; object-fit:cover;
                border:2.5px solid rgba(255,255,255,0.25); flex-shrink:0;
    ...
    <div class="profile-info">
        ...
        <div class="profile-cols">
            ...
            <div class="profile-col">
                <h4>수상이력</h4>
                <details>
                    <summary style="cursor:pointer; list-style:none; font-size:0.9rem; color:#e0daf5;">
                        🏆 스파르타 최종 프로젝트 최우수상 (2026.03)
                        <span class="hint"><br/>▶ 펼쳐보기</span>
                    </summary>
                    <img src="data:image/png;base64,{cert4_b64}" style="width:380px; margin-top:0.5rem; border-radius:0.5rem;">
                </details>
            </div>
        </div>
    </div>
...
with col1:
    st.markdown(f"""
    <div class="card-box">
        <h4 style="color:#533483; font-size:0.78rem; text-transform:uppercase; letter-spacing:1.8px; margin:0 0 1rem 0; font-weight:700;">자격증</h4>
        <details style="margin-bottom:0.6rem;">
            <summary style="cursor:pointer; list-style:none; font-size:0.97rem; color:#2b2244; font-weight:500;">
...
with col2:
    st.markdown("""
    <div class="card-box">
        <h4 style="color:#533483; font-size:0.78rem; text-transform:uppercase; letter-spacing:1.8px; margin:0 0 1rem 0; font-weight:700;">경력</h4>
        <details style="margin-bottom:0.6rem;">
            <summary style="cursor:pointer; list-style:none; font-size:0.97rem; color:#2b2244; font-weight:500;">
                🏢 스파르타 내일배움캠프 데이터분석가 과정 | 학생 (2025.10 ~ 2026.3)<span class="hint">▶ 펼쳐보기</span>
            </summary>
            <div style="margin-top:0.6rem; font-size:0.92rem; color:#444; line-height:1.85; padding-left:0.8rem;">
...
with col1:
    st.markdown(f"""
    <div class="proj-card" style="border-top: 4px solid #d0021b;">
        <div style="display:flex; align-items:center; gap:0.7rem; margin-bottom:0.8rem;">
            {daiso_logo}
            <strong style="font-size:1.05rem; color:#1a1a2e;">초저가를 넘어 초신뢰로</strong>
            <span style="margin-left:auto; font-size:0.85rem; color:#d0021b; font-weight:600;">🏆 최우수상</span>
        </div>
        <p style="font-size:0.92rem; color:#555; margin-bottom:1rem; line-height:1.65;">다이소 뷰티 30만 건 리뷰 분석을 통한 2026년 성장 전략 도출</p>
        ...
        <div style="display:flex; gap:2rem; padding-top:1rem; border-top:1px solid #f5f2fc;">
            <div style="text-align:center;">
                <div style="font-size:1.25rem; font-weight:700; color:#d0021b;">30만건</div>
                <div style="font-size:0.8rem; color:#aaa; margin-top:0.2rem;">리뷰 데이터</div>
            </div>
            <div style="text-align:center;">
                <div style="font-size:1.25rem; font-weight:700; color:#d0021b;">83.3%</div>
                <div style="font-size:0.8rem; color:#aaa; margin-top:0.2rem;">연착륙 제품 중 스킨케어</div>
            </div>
            <div style="text-align:center;">
                <div style="font-size:1.25rem; font-weight:700; color:#d0021b;">144%</div>
                <div style="font-size:0.8rem; color:#aaa; margin-top:0.2rem;">뷰티 카테고리 성장률</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with col2:
    ...
    <div class="proj-card" style="border-top: 4px solid #00704a;">
        <div style="display:flex; align-items:center; gap:0.7rem; margin-bottom:0.8rem;">
            {starbucks_logo}
            <strong style="font-size:1.05rem; color:#1a1a2e;">Starbucks Next Level</strong>
        </div>
        <p style="font-size:0.92rem; color:#555; margin-bottom:1rem; line-height:1.65;">행동경제학 기반 프로모션 채널 효과 분석 및 고객 세그먼트 전략 수립</p>
        <div style="display:flex; gap:0.35rem; flex-wrap:wrap; margin-bottom:1.2rem;">
        ...
        <div style="display:flex; gap:2rem; padding-top:1rem; border-top:1px solid #f0faf5;">
            <div style="text-align:center;">
                <div style="font-size:1.25rem; font-weight:700; color:#00704a;">43.97%p</div>
                <div style="font-size:0.8rem; color:#aaa; margin-top:0.2rem;">SNS 채널 효과</div>
            </div>
            <div style="text-align:center;">
                <div style="font-size:1.25rem; font-weight:700; color:#00704a;">4개</div>
                <div style="font-size:0.8rem; color:#aaa; margin-top:0.2rem;">고객 세그먼트</div>
            </div>
            <div style="text-align:center;">
                <div style="font-size:1.25rem; font-weight:700; color:#00704a;">3가지</div>
                <div style="font-size:0.8rem; color:#aaa; margin-top:0.2rem;">행동경제학 인사이트</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
def render_skills(items):
    for icon, name, pct in items:
        ...
        <div class="skill-card">
            ...
            <div style="margin-left:auto; font-size:1.05rem; letter-spacing:2px;
                        text-align:right; display:flex; align-items:center; gap:0.4rem;">
                <span style="font-size:0.78rem; color:#bbb;">숙련도</span>
                <span style="color:#e94560;">{filled}</span><span style="color:#e8e0f5;">{empty}</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
